#Faça um programa que peça o tamanho de um arquivo para download (em MB) e a velocidade de um link de
# Internet (em Mbps), calcule e informe o tempo aproximado de download do arquivo usando este link (em minutos).
import math

arquivo = int(input('Insira o tamanho do arquivo em (MB): '))
velocidade = int(input('Insira a velocidade da sua intenret em (Mbps): '))
tempo = math.ceil(arquivo / velocidade)

print(f'O tempo aproximado de download do seu arquivo é de {tempo} minutos.')